...
First, run [solarpos-cli](https://github.com/KlausBrunner/solarpos-cli) and capture its output in a CSV file. We're getting position data for the entire year 2023 in Rome, Italy. Depending on your setup, you may have to add the "java" command and an absolute path.
...
Doesn't have the same mysterious vibe as the previous ones, but it's a bit more useful now. Of course, don't forget all times are UTC. You can also tell solarpos-cli to use the proper local timezone (Europe/Rome), which will include the DST transitions.
